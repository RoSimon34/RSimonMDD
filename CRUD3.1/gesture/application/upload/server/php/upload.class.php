...
class UploadHandler
{
	...
	{
		$delete_from_db = $this->query("DELETE FROM images WHERE id = '$delimg'") or die(mysql_error());
    ...
    function query($query) {
	    ...
	    $db = new \PDO("mysql:hostname=127.0.0.1;port=8889;dbname='.$database.'", "root", "root");
	    $st = $db->prepare($query);
	    $result = $st->execute();
	    mysql_close($link);
	    return $result;
	...
	/*
public function add($name='', $desc='', $quantity='', $date=''){
		
		$db = new \PDO("mysql:hostname=127.0.0.1;port=8889;dbname=ssl1210", "root", "root");
		
		$sql = "insert into products(pname, pdesc, quantity, add_date) values (:pname, :pdesc, :quantity, :date)";
		
		$st = $db->prepare($sql);
		
		$results = $st->execute(array(":pname"=>$name, ":pdesc"=>$desc, ":quantity"=>$quantity, ":date"=>$date));
...
*/
	...
	{
		echo 'wtf';
		//$add_to_db = $this->query("INSERT INTO images (image_name) VALUES ('".$whichimg."')") or die(mysql_error());
		$db = new \PDO("mysql:hostname=127.0.0.1;port=8889;dbname=glmt", "root", "root");
		
		$sql = 'insert into images(image_name) values('"hello"')';
		
		$st = $db->prepare($sql);
		
		$results = $st->execute(array(":imgname"=>$whichimg));

...
/*
		$data['projects']=$this->files_model->get_projects();
*/
	...
	{	
		if(($this->session->userdata('user_name')!=""))
		{
			...
			//$data['individualP']=$this->files_model->individualProject();
	...
	//this function loads the model and gets all the results from the database, next it goes to the view.
	public function commment_view()
	{
      $this->load->model('commentjquery');
      $data['result'] = $this->commentjquery->get_messages();
      var_dump($data);
      $this->load->view('registration_view',$data);
    }
    //this function loads the form validation library and checks all the inputs if the it passses it goes to the index
    public function comments()
    {

		$this->load->library('form_validation');
		// field name, error message, validation rules
		$this->form_validation->set_rules('username', 'Your Name', 'trim|required|min_length[4]|xss_clean');
		...
		{
			$this->commentjquery->add_message();
			$this->index();
		}
     }
